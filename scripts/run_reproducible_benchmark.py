...
DEFAULT_BUNDLE = ROOT / "benchmark" / "reproducible"
PACKAGE_SOURCES = {
    "inputs/mlat.jsonl": ROOT / "benchmark" / "fixtures" / "reproducible-mlat.jsonl",
    ...
    "source/reproducibility-workflow.yml": ROOT
    / ".github"
    / "workflows"
    / "reproducibility.yml",
    ...
    for relative, source in PACKAGE_SOURCES.items():
        if not source.is_file():
            raise FileNotFoundError(
                f"Required reproducibility source is missing: {source}"
            )
...
def artifact_metadata(
    bundle: Path, relative_paths: set[str]
...
def build_bundle(bundle: Path) -> dict[str, Any]:
    ...
    checksums = "\n".join(
        f"{sha256(bundle / relative)}  {relative}"
        for relative in sorted(checksum_targets)
    )
...
def verify_bundle(bundle: Path) -> dict[str, Any]:
    ...
    if (
        manifest.get("deterministic") is not True
        or manifest.get("live_validation") is not False
    ):
    ...
    if (
        ...
        raise ValueError(
            "Synthetic fixture was incorrectly labelled as benchmarkable or live"
        )
