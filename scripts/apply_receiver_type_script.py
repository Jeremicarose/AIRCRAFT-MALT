...
from network.receiver_registry import calculate_type_id, normalize_identity_id
...
def creation_identity(transaction: dict, output_index: int) -> str:
    ...
    if not inputs:
        raise SystemExit("Registry V2 creation requires a funding input before applying the type script")
    first = inputs[0]
