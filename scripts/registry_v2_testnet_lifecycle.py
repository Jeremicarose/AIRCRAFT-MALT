...
from network.receiver_registry import ReceiverRegistryRecord, calculate_type_id
...
def ckb_hash(data: bytes) -> str:
    return "0x" + hashlib.blake2b(
        data, digest_size=32, person=b"ckb-default-hash"
    ).hexdigest()
...
def main() -> None:
    ...
    write_tx(
        ...
        outputs=[
            output(REGISTRY_CAPACITY, owner_a["lock_arg"], type_script(contract_code_hash, "0x" + "ff" * 32)),
