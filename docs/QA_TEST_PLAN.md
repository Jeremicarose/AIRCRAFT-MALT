...
|---|---|---|---|
| Open application | Receiver directory is the first screen | PASS | Root and `/app` redirect to `/app/registry`; the production build includes that route |
...
| Frontend receiver state | Invalid, future, and stale observations fail closed | PASS | 4 focused frontend tests passed |
