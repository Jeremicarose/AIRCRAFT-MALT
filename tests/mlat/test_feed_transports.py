...
def test_simulation_feed_dispatches_scenario_aircraft_to_receivers():
    ...
    async def observe_first_message(receiver_id, timestamp, message, **timing):
        received.append((receiver_id, timestamp, message, timing))
        raise FeedObserved
    ...
    assert received[0][3]["timestamp_ns"] > 0
    assert received[0][3]["clock_synchronized"] is True


def test_json_feed_preserves_precision_and_clock_qualification():
    timestamp_ns = 1_710_000_000_123_456_789
    ...
    records = JsonFeedParsingMixin().normalize_feed_records(
        {
            ...
            "clock_source": "gps-disciplined-beast",
