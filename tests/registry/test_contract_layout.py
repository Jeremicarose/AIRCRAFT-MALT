...
def test_contract_skeleton_exists():
    root = Path("contracts/registry-v2")
...
def test_contract_mentions_canonical_schema_fields():
    record_rs = Path("contracts/registry-v2/src/record.rs").read_text()
    for field in [
        "receiver_id",
        "latitude",
        "longitude",
...
def test_contract_has_transaction_level_lifecycle_tests():
    lifecycle_test = Path("contracts/registry-v2/tests/lifecycle.rs").read_text()
    for behavior in [
