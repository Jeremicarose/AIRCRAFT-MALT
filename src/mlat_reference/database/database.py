...
class MLATDatabase:
    ...
    def _seed_default_plans(self, cursor: sqlite3.Cursor):
        ...
        default_plans = [
            (
                ...
                "Public Demo",
                "public",
                "Public demo access with shallow history and no premium outputs.",
                900,
                0,
                0,
                created_at,
            ...
            (
                ...
                "Premium",
                "premium",
                "Premium access to quality-enriched outputs, live streams, and deeper history.",
                86400,
                1,
                1,
                created_at,
    ...
    def get_aircraft_track(
        ...
        if not rows:
            return AircraftTrack(
                aircraft_id=aircraft_id, start_time=0, end_time=0, num_positions=0, positions=[]
            )
    ...
    def get_recent_positions(self, seconds: int = 60, limit: int = 100) -> List[StoredPosition]:
        """
    ...
    def store_statistics(
        ...
        for column_name, definition in migrations.items():
            if column_name not in columns:
                cursor.execute(f"ALTER TABLE statistics ADD COLUMN {column_name} {definition}")
    ...
    def store_receiver(
        ...
        for column_name, definition in migrations.items():
            if column_name not in columns:
                cursor.execute(f"ALTER TABLE receivers ADD COLUMN {column_name} {definition}")
