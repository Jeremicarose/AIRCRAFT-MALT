...
from ckb_registry.discovery import CKBPeerDiscovery, CKBConfig, ReceiverInfo
from mlat_reference.demo import build_demo_receivers
...
class CKBReceiverNetworkClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info("CKB Registry V2 discovery connected")
        except Exception:
            if self.config.strict_production_mode or not self.config.simulate_if_unavailable:
                raise
            receivers = []
            logger.warning(
                "CKB discovery unavailable; the MLAT reference will use explicit demo receivers",
                exc_info=True,
            )
    ...
    def _get_simulated_receivers(self) -> List[ReceiverInfo]:
        """Build deterministic receivers owned by the MLAT demo adapter."""
