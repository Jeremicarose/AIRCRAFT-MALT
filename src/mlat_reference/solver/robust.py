"""Precision-aware TDOA multilateration solver."""
...
class RobustMLATSolver:
    """Solve receiver TDOA observations with damped least squares."""

    def __init__(self, min_receivers: int = 4):
        self.min_receivers = min_receivers
        self.max_iterations = 100
        self.convergence_threshold = 0.01  # RMS range-difference residual, metres
    ...
    def solve_position(self, observations: List[SignalObservation]) -> Optional[AircraftPosition]:
        ...
        timing = self._relative_arrival_times(observations)
        if timing is None:
            return None

        observations, time_diffs, correlation_time_span_s = timing
        ...
        return AircraftPosition(
            ...
            correlation_time_span_s=correlation_time_span_s,
            receiver_count=len(observations),
            timestamp_ns=ref_obs.timestamp_ns,
        )

    def _relative_arrival_times(
        self,
        observations: List[SignalObservation],
    ) -> Optional[Tuple[List[SignalObservation], np.ndarray, float]]:
        """Return precision-safe arrival differences relative to the earliest receiver."""
        precise_count = sum(obs.timestamp_ns is not None for obs in observations)
        if precise_count not in {0, len(observations)}:
            logger.debug("Mixed precision and legacy timestamps are not comparable")
            return None

        if precise_count == len(observations):
            ordered = sorted(observations, key=lambda obs: int(obs.timestamp_ns))
            reference_ns = int(ordered[0].timestamp_ns)
            deltas_ns = np.array(
                [int(obs.timestamp_ns) - reference_ns for obs in ordered[1:]],
                dtype=np.float64,
            )
            span_ns = int(ordered[-1].timestamp_ns) - reference_ns
            return ordered, deltas_ns / 1_000_000_000.0, span_ns / 1_000_000_000.0

        # Small relative values remain useful for isolated algorithm tests and
        # legacy callers. Unix-epoch floats cannot preserve MLAT precision.
        if any(
            not np.isfinite(obs.timestamp) or abs(obs.timestamp) >= 1_000_000
            for obs in observations
        ):
            logger.debug("Epoch timestamps require integer timestamp_ns metadata")
            return None

        ordered = sorted(observations, key=lambda obs: obs.timestamp)
        reference_s = ordered[0].timestamp
        deltas_s = np.array(
            [obs.timestamp - reference_s for obs in ordered[1:]],
            dtype=np.float64,
        )
        return ordered, deltas_s, ordered[-1].timestamp - reference_s
    ...
    def _get_smart_initial_guess(
        self, ref_pos: np.ndarray, positions: np.ndarray, time_diffs: np.ndarray
    ) -> Optional[np.ndarray]:
        """
        Get smart initial guess using geometric centroid
        with altitude based on typical aircraft cruising altitude.
        """
        # With five or more receivers, solve the linearized TDOA equations for
        # position plus range to the reference receiver. This is only an
        # initial estimate; damped least squares enforces the nonlinear model.
        if len(positions) >= 4:
            range_diffs = time_diffs * SPEED_OF_LIGHT
            matrix_rows = []
            right_hand_side = []
            ref_norm_squared = float(np.dot(ref_pos, ref_pos))
            for position, range_diff in zip(positions, range_diffs):
                matrix_rows.append(np.concatenate((ref_pos - position, [-range_diff])))
                right_hand_side.append(
                    (range_diff**2 - float(np.dot(position, position)) + ref_norm_squared) / 2.0
                )
            try:
                ...
                if rank >= 4 and np.all(np.isfinite(solution[:3])):
                    return solution[:3]
            except np.linalg.LinAlgError:
                pass

        # Four receivers provide the minimum three TDOA equations. Use the
        # receiver centroid's geodetic direction and a physical altitude prior.
        all_positions = np.vstack([ref_pos.reshape(1, 3), positions])
        centroid = np.mean(all_positions, axis=0)
        latitude, longitude, _altitude = self._ecef_to_lla(centroid)
        return ReceiverPosition(latitude, longitude, 9_000.0, "INITIAL").to_ecef()
    ...
    def _levenberg_marquardt(
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                # J is the derivative of the residual, so Gauss-Newton solves
                # J * delta = -residual.
        ...
        if best_residual < 10000:  # 10km threshold
            ...
            return best_x, best_residual, best_iteration
    ...
    def _estimate_uncertainty(
        self, ref_pos: np.ndarray, positions: np.ndarray, aircraft_pos: np.ndarray, residual: float
    ) -> float:
        """
