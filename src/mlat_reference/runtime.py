...
from mlat_reference.correlation.correlator import RawSignal
...
from mlat_reference.demo import get_demo_scenario
from mlat_reference.base import BaseMLATRuntime
from mlat_reference.config import DemoSettings, RuntimeSettings, load_runtime_settings
...
class ProductionMLATSystem(BaseMLATRuntime[ReceiverPosition, SignalObservation]):
    """
    Single-node MLAT reference runtime with integrated ingest and persistence.
    """
    ...
    def __init__(
        ...
        self.simulation_mode = (
            config.simulate_if_unavailable and not config.receiver_registry_type_hash
        )
    ...
    def get_runtime_state(self) -> dict:
        ...
        return {
            ...
            "last_successful_solve_age_s": (
                max(0.0, now - self.last_successful_solve_at)
                if self.last_successful_solve_at
                else None
            ),
            "last_store_at": self.last_store_at,
            "last_store_age_s": (
                max(0.0, now - self.last_store_at) if self.last_store_at else None
            ),
            ...
            "max_solve_latency_ms": (
                max(self.solve_latencies_ms) if self.solve_latencies_ms else 0.0
            ),
            "avg_store_latency_ms": (
                sum(self.store_latencies_ms) / len(self.store_latencies_ms)
                if self.store_latencies_ms
                else 0.0
            ),
            "max_store_latency_ms": (
                max(self.store_latencies_ms) if self.store_latencies_ms else 0.0
            ),
            "avg_api_latency_ms": (
                sum(self.api_latencies_ms) / len(self.api_latencies_ms)
                if self.api_latencies_ms
                else 0.0
            ),
    ...
    def _signal_clock_is_qualified(self, signal: RawSignal) -> bool:
        return bool(
            ...
            and signal.clock_source.strip().lower() not in {"", "unknown", "network-arrival"}
        ...
        try:
            ...
            self.database.store_position(
                ...
                solver_method=getattr(
                    position, "solver_method", getattr(position, "method", "unknown")
                ),
                solver_residual_m=getattr(
                    position, "solver_residual_m", getattr(position, "residual", 0.0)
                ),
                solver_iterations=getattr(
                    position, "solver_iterations", getattr(position, "iterations", 0)
                ),
    ...
    async def _statistics_loop(self):
        ...
        while self.is_running:
            ...
            logger.info(
                "  Clock-qualified receivers: " f"{runtime_state['clock_synchronized_receivers']}"
            )
            logger.info("=" * 70)
