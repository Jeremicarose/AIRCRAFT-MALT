...
def load_app_config() -> Dict[str, object]:
    ...
    return {
        ...
        "BENCHMARK_REPORT_PATH": os.getenv(
            "BENCHMARK_REPORT_PATH", "evidence/mlat-reference/live/latest.json"
        ),
...
def _validate_api_key() -> bool:
    ...
    return bool(supplied) and hmac.compare_digest(str(supplied), str(api_key))
...
def _request_access_policy(auth: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    if auth is not None:
        return auth

    public_plan = get_db().get_plan_by_code(current_app.config["PUBLIC_PLAN_CODE"])
    if public_plan is None:
        raise RuntimeError("Configured public API plan does not exist")
    return {
        "max_history_seconds": public_plan.max_history_seconds,
        "can_access_premium": public_plan.can_access_premium,
    }


def _positive_int_query(name: str, default: int) -> int:
...
def _optional_finite_float_query(name: str) -> Optional[float]:
    raw_value = request.args.get(name)
    if raw_value is None:
        return None
    try:
...
def _has_active_entitlement(auth: Dict[str, Any], entitlement_code: str) -> bool:
    ...
    for entitlement in auth.get("entitlements", []):
        if entitlement.get("entitlement_code") != entitlement_code:
            continue
        if entitlement.get("status") != "active":
            continue
        try:
            starts_at = _parse_entitlement_timestamp(entitlement.get("starts_at"))
            ends_at = _parse_entitlement_timestamp(entitlement.get("ends_at"))
...
def _parse_entitlement_timestamp(value: Optional[str]) -> Optional[float]:
    if value is None:
        return None
    if not isinstance(value, str) or not value.strip():
        raise ValueError("entitlement timestamp must be a non-empty ISO-8601 string")
...
def record_usage(
    event_type: str, resource: str, quantity: int = 1, metadata: Optional[Dict[str, Any]] = None
):
    auth = get_request_auth()
    if auth is None:
        return
...
def get_runtime_state(db: Optional[MLATDatabase] = None) -> Optional[Dict[str, object]]:
    ...
    if runtime is not None:
        ...
        last_store_age_s = (
            max(0.0, now - latest_position.timestamp) if latest_position is not None else None
        )
        signal_fresh = bool(
        ...
        state = {
            ...
            "max_clock_uncertainty_ns": float(latest_stats.get("max_clock_uncertainty_ns", 0.0)),
        ...
        state["signals_per_second"] = int(state["total_signals"]) / uptime_s if uptime_s else 0.0
        state["positions_per_second"] = total_positions / uptime_s if uptime_s else 0.0
        state["solve_success_percent"] = (
            100.0 * int(state["successful_solves"]) / total_positions if total_positions else 0.0
        )
    else:
        return None
        ...
        samples.append(
            {
                ...
                "solve_success_percent": (
                    round(100.0 * successful_solves / total_positions, 2)
                    if total_positions
                    else 0.0
                ),
...
def health_check():
    ...
    status = "ok"
    if (
        last_signal_age_s is None
        or last_signal_age_s > current_app.config["HEALTH_STALE_SIGNAL_SECONDS"]
    ):
        status = "degraded"
    ...
    return jsonify(
        {
            ...
            "freshness": {
                ...
                "receiver_stale_count": sum(
                    1
                    for age in receiver_freshness
                    if age > current_app.config["HEALTH_STALE_SIGNAL_SECONDS"]
                ),
            },
            "broadcaster": {
...
def readiness_check():
    ...
    simulated_positions = sum(
        1 for position in recent_positions if _is_synthetic_solver_method(position.solver_method)
    )
    synthetic_feed = bool(
        runtime_state.get(
            "synthetic_feed_mode",
    ...
    benchmarkable_output = position_count > 0 and simulated_positions == 0 and not synthetic_feed
    ...
    clock_synchronized_receivers = int(runtime_state.get("clock_synchronized_receivers", 0))
...
def get_public_evidence_metrics():
    ...
    current["avg_api_latency_ms"] = round(float(runtime_state.get("avg_api_latency_ms", 0.0)), 3)
    ...
    fresh_samples = [item for item in history if item.get("last_signal_age_s") is not None]
    fresh_signal_samples = sum(
        1
        for item in fresh_samples
        if float(item["last_signal_age_s"])
        <= int(current_app.config["HEALTH_STALE_SIGNAL_SECONDS"])
    )
    ...
    return jsonify(
        {
            ...
            "reliability": {
                "receiver_availability_percent": (
                    round(100.0 * receiver_ready_samples / sample_count, 2)
                    if sample_count
                    else None
                ),
                "signal_freshness_percent": (
                    round(100.0 * fresh_signal_samples / len(fresh_samples), 2)
                    if fresh_samples
                    else None
                ),
...
def get_pipeline_evidence():
    ...
    total_positions = int(
        runtime_state.get("total_positions", latest_stats.get("total_positions", 0))
    )
    ...
    registry_status = (
        "pass" if registry_configured and registry_discovery_live else "demo" if demo else "blocked"
    )
    discovery_status = (
        "pass"
        if registry_configured and registry_discovery_live and receivers
        else "demo" if demo and receivers else "waiting"
    )
    ingest_status = (
        "demo"
        if synthetic and signal_fresh
        else "pass" if signal_fresh and total_signals > 0 else "waiting"
    )
    processing_status = (
        "demo"
        if synthetic and total_positions > 0
        else "pass" if total_positions > 0 else "waiting"
    )
    clock_synchronized_receivers = int(runtime_state.get("clock_synchronized_receivers", 0))
    clock_ready = clock_synchronized_receivers >= 4
    clock_status = (
        "demo"
        if synthetic and clock_ready
        else "pass" if not synthetic and clock_ready else "blocked" if signal_fresh else "waiting"
    )
    solve_status = (
        "demo"
        if synthetic and latest_position
        else "pass" if latest_position and benchmarkable else "waiting"
    )
    ...
    stages = [
        stage(
            ...
            registry_status,
            (
                "Receiver cells were discovered through the configured CKB type hash."
                if registry_discovery_live
                else (
                    "Demo receivers are local; configure the CKB registry type hash for live proof."
                    if demo
                    else (
                        "A type hash is configured, but live CKB discovery is not verified."
        ...
        stage(
            ...
            ingest_status,
            (
                "Receiver observations are arriving."
                if signal_fresh
        ...
        stage(
            ...
            processing_status,
            (
                "Correlated groups are reaching the solve path."
                if total_positions
        ...
        stage(
            ...
            solve_status,
            (
                "Replay arrival times were localized by the robust MLAT solver; replay provenance prevents live benchmark claims."
                if synthetic and latest_position
                else (
                    "Robust MLAT positions are available."
                    if latest_position
        ...
        stage(
            "storage",
            "Position storage",
            "pass" if latest_position else "waiting",
            (
                "Latest position is persisted in SQLite."
                if latest_position
        ...
        stage(
            ...
            pipeline_route="/app/pipeline",
            metrics_route="/app/analytics",
        ),
    ]
...
def landing_page():
    return jsonify(
        {
            "project": "CKB Registry V2",
            "component": "MLAT reference API",
            "api": "/api",
            "health": "/api/health",
            "frontend": "Run reference/mlat/frontend as a separate service.",
        }
    )
...
def dashboard_page():
    return jsonify({"error": "The MLAT reference frontend is a separate service."}), 404
...
def get_system_mode():
    ...
    runtime_status = (
        ...
        else "stale" if runtime_state else "unavailable"
    )
    ...
    benchmarkable_output = bool(
        ...
        and all(
            not _is_synthetic_solver_method(position.solver_method) for position in recent_positions
        )
    )
    startup_guardrail_status = (
        ...
        if strict_production_mode and runtime_status == "active"
            ...
            else "demo" if demo_mode else "simulation" if configured_simulation else "standard"
        )
    )
    return jsonify(
        {
            "mode": (
                ...
                if demo_mode
                    ...
                    else "live" if runtime_status == "active" else "configured_live"
                )
            ),
            ...
            "registry_code_immutable": os.getenv("RECEIVER_REGISTRY_HASH_TYPE", "data1") == "data1",
            "registry_discovery_live": bool(runtime_state.get("registry_discovery_live")),
...
def get_recent_positions():
    auth = get_request_auth()
    access = _request_access_policy(auth)
    try:
        seconds = _positive_int_query("seconds", 60)
        limit = _positive_int_query("limit", 100)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    seconds = min(seconds, access["max_history_seconds"])
    if limit > 100 and not access.get("can_access_premium", False):
        return (
            jsonify({"error": "Premium plan required for deeper recent-position queries"}),
            403,
        )
    if limit > 1000:
        return jsonify({"error": "'limit' must not exceed 1000"}), 400
    ...
    record_usage(
        "rest_request",
        "recent_positions",
        quantity=len(positions_data),
        metadata={"seconds": seconds, "limit": limit},
    )
    return jsonify(
        {
...
def get_aircraft_track(aircraft_id: str):
    auth = get_request_auth()
    access = _request_access_policy(auth)
    default_limit = 1000 if access.get("can_access_premium", False) else 250
    try:
        start_time = _optional_finite_float_query("start_time")
        end_time = _optional_finite_float_query("end_time")
        limit = _positive_int_query("limit", default_limit)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    if limit > 250 and not access.get("can_access_premium", False):
        return jsonify({"error": "Premium plan required for deep track history"}), 403
    if limit > 5000:
        return jsonify({"error": "'limit' must not exceed 5000"}), 400

    effective_end = end_time if end_time is not None else time.time()
    effective_start = (
        start_time if start_time is not None else effective_end - access["max_history_seconds"]
    )
    if effective_start > effective_end:
        return jsonify({"error": "'start_time' must not exceed 'end_time'"}), 400
    if effective_end - effective_start > access["max_history_seconds"]:
    ...
    record_usage(
        "rest_request",
        "aircraft_track",
        quantity=track.num_positions,
        metadata={"aircraft_id": aircraft_id, "limit": limit},
    )
    return jsonify(
        {
...
def handle_subscribe(data):
    ...
    if not api_key:
        emit(
            ...
            {"aircraft_id": aircraft_id, "status": "denied", "error": "API key required"},
        )
        return
    ...
    if auth is None or auth["api_key_status"] != "active" or auth["account_status"] != "active":
        emit(
            "subscription_response",
            {"aircraft_id": aircraft_id, "status": "denied", "error": "Invalid API key"},
        )
        return
    if not auth.get("can_stream_live", False):
        emit(
            "subscription_response",
            {"aircraft_id": aircraft_id, "status": "denied", "error": "Streaming plan required"},
        )
        return
    if not _has_active_entitlement(auth, "live_stream"):
        emit(
            "subscription_response",
            {
                "aircraft_id": aircraft_id,
                "status": "denied",
                "error": "Live stream entitlement required",
            },
        )
        return
