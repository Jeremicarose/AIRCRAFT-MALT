...
def aircraft_state_at(timestamp: float, aircraft: DemoAircraftDefinition) -> Dict[str, float | str]:
    ...
    cycle_position = ((timestamp + aircraft.phase_offset) % aircraft.period_seconds) / aircraft.period_seconds
...
def scenario_aircraft_states(timestamp: float, scenario_name: str | None) -> List[Dict[str, float | str]]:
