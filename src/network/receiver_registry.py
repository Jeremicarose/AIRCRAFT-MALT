...
class ReceiverRegistryRecord:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.receiver_id, str) or not _RECEIVER_ID_RE.fullmatch(
            self.receiver_id
        ):
        ...
        if any(isinstance(value, bool) or not math.isfinite(float(value)) for value in coordinates):
        ...
        if self.metadata_hash is not None and not _METADATA_HASH_RE.fullmatch(
            self.metadata_hash
        ):
