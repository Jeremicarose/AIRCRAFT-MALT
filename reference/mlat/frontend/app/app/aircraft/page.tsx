...
export default async function AircraftRoute({ searchParams }: { searchParams: Promise<{ aircraft?: string }> }) {
  ...
  const [snapshot, positionsState] = await Promise.all([
    fetchShellSnapshot(),
    fetchJsonState<PositionsResponse>(PUBLIC_POSITIONS_PATH, { positions: [] }),
  ]);
  const positionsData = positionsState.data;
