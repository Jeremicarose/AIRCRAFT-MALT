...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-4 max-w-6xl">
      <WorkspaceHeader title="Operator preferences" description="Choose local refresh, notification, layout, and map defaults for this browser." rail={<SignalMarquee items={[{ label: 'Refresh', value: `${refreshInterval}s`, tone: 'selection' }, { label: 'Alerts', value: desktopAlerts ? 'Enabled' : 'Disabled', tone: desktopAlerts ? 'healthy' : 'attention' }, { label: 'Density', value: density, tone: density === 'compact' ? 'selection' : 'neutral' }, { label: 'Permission', value: notificationPermission, tone: notificationPermission === 'granted' ? 'healthy' : notificationPermission === 'default' ? 'attention' : 'failure' }]} />} />
      <WorkspaceSplit
