...
export default function LiveMapPage({ initialModeData, initialHealthData, initialPositionsData, initialPositionsError, initialReceiversData, initialSelectedAircraftId, initialSelectedReceiverId }: LiveMapPageProps) {
  ...
  const positionsQuery = useQuery({ queryKey: ['positions', 'live-map'], queryFn: () => fetchJson<PositionsResponse>(PUBLIC_POSITIONS_PATH), initialData: initialPositionsData, refetchInterval: refreshInterval * 1_000 });
  ...
  const activityRail = [
    { title: 'Position updates', detail: streamConnected ? 'Websocket updates are connected.' : `Polling every ${refreshInterval} seconds.`, meta: streamConnected ? 'Live' : 'Polling', tone: streamConnected ? 'healthy' : 'attention' as StatusTone },
  ...
  return (
    <div className="space-y-4">
      {(positionsQuery.error instanceof Error || (initialPositionsError && !positionsQuery.isFetchedAfterMount)) ? <DataNotice title="The air picture could not be loaded" detail={positionsQuery.error instanceof Error ? positionsQuery.error.message : initialPositionsError ?? 'The latest position request failed.'} onRetry={() => void positionsQuery.refetch()} /> : null}
      <WorkspaceHeader title="Network map" description={isReplay ? 'Explore the replay receiver network and solved aircraft. Replay data demonstrates the MLAT workflow but is not live CKB evidence.' : 'Select an aircraft or receiver to keep its location, status, and evidence in the shared inspector.'} status={<StatusChip label={isReplay ? 'Replay data' : streamConnected ? 'Live stream' : modeQuery.data?.runtime_status === 'active' ? 'Live polling' : 'Offline'} tone={isReplay ? 'replay' : streamConnected || modeQuery.data?.runtime_status === 'active' ? 'healthy' : 'failure'} />} actions={<button type="button" onClick={() => void refresh()} disabled={positionsQuery.isFetching} className={buttonVariants({ variant: 'primary', size: 'sm' })}><RefreshCw className={cn('size-3.5', positionsQuery.isFetching && 'animate-spin')} />{positionsQuery.isFetching ? 'Refreshing' : 'Refresh map'}</button>} rail={<SignalMarquee items={[{ label: 'Aircraft', value: number(aircraft.length), tone: aircraft.length ? 'healthy' : 'attention' }, { label: 'Receivers', value: number(receivers.length), tone: receivers.length ? 'selection' : 'attention' }, { label: 'Transport', value: streamConnected ? 'Websocket' : 'Polling', tone: streamConnected ? 'healthy' : 'attention' }, { label: 'Network', value: 'CKB testnet', tone: 'trust' }]} />}/>
