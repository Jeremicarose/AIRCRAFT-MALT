...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bridge raw local feed sources into MLAT command-jsonl format"
    )
    parser.add_argument(
        "--source",
        required=True,
...
def normalize_timestamp_ns(value: Any) -> int | None:
...
def normalize_record(
    payload: Any, receiver_map: dict[str, str], default_receiver_id: str | None
) -> list[dict[str, Any]]:
    ...
    timestamp_ns = normalize_timestamp_ns(payload.get("timestamp_ns"))
    return [
        {
            ...
            "clock_uncertainty_ns": payload.get("clock_uncertainty_ns"),
            "message": str(message),
        }
    ]
...
async def read_tcp(args: argparse.Namespace, receiver_map: dict[str, str]) -> None:
    ...
    while True:
        ...
        if not line:
            return
        emitted_total += await emit_from_line(
            line.decode("utf-8"), receiver_map, args.default_receiver_id
        )
        if args.once and emitted_total:
            return
