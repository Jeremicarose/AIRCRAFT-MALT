...
from mlat_reference.database.database import MLATDatabase  # noqa: E402
...
def benchmark_database(receiver_count: int, insert_count: int) -> dict[str, dict[str, float | int]]:
    with tempfile.TemporaryDirectory(prefix="mlat-benchmark-") as directory:
        ...
        lookup_times = [
            timed_ms(database.get_receivers) for _ in range(max(50, receiver_count * 4))
        ]
...
```bash
python3 tools/mlat/benchmark_operational_performance.py --api-base http://127.0.0.1:5057
```
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default="evidence/mlat-reference/experimental/performance-local-simulation.json",
    )
    parser.add_argument(
        "--markdown",
        default="evidence/mlat-reference/experimental/PERFORMANCE.md",
    )
