...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export MLAT positions into benchmark JSONL format"
    )
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        for position in positions:
            record = {
                ...
                "solver_residual_m": row_value(
                    position, "solver_residual_m", row_value(position, "residual", 0.0)
                ),
                "receiver_count": row_value(
                    position, "receiver_count", row_value(position, "num_receivers", 0)
                ),
