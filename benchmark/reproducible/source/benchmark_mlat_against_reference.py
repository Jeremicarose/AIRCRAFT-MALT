...
def build_report(
    ...
    *,
    ...
    if any(method.strip().lower() in NON_LIVE_SOLVER_METHODS for method in solver_methods):
        limitations.append("The MLAT dataset contains simulated, unknown, or unlabelled solver provenance.")
    ...
    freshness = {
        ...
        "end_to_store_age_median_ms": statistics.median(end_to_store_ages_ms) if end_to_store_ages_ms else None,
        "end_to_store_age_p95_ms": percentile(end_to_store_ages_ms, 95) if end_to_store_ages_ms else None,
    }
    matched = len(matches)
